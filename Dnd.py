...
def init_user_session():
    ...
    if "user_type" not in st.session_state:
        st.session_state.user_type = "guest"  # guest, player, host
    ...
    # Регистрируем пользователя в общих данных
...
def registration_interface():
    ...
    st.title("⚔️ D&D Spell Caster - Регистрация")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.header("🎮 Игрок")
        player_name = st.text_input("Введите имя игрока", key="reg_player_name")
        
        if st.button("Войти как игрок", use_container_width=True, type="primary"):
            if player_name:
                st.session_state.user_name = player_name
                st.session_state.user_type = "player"
                # Регистрируем в общих данных
                shared_data["users"][st.session_state.user_id] = {
                    "name": player_name,
                    "type": "player",
                    "last_active": time.time()
    ...
    st.info("""
    ### 📝 Как это работает:
    1. **Игроки** регистрируются с именем
    2. **Мастер** входит с паролем `admin123`
    3. Все действия синхронизируются в реальном времени
    4. Общий стол игроков виден всем
    """)
...
def player_interface():
    ...
    st.title(f"🧙‍♂️ Игрок: {st.session_state.user_name}")
    ...
    with col_search:
        ...
        # Всегда показываем кнопки (но некоторые disabled)
        st.markdown("---")
        st.write("### Действия:")
        ...
        if selected_spell:
            ...
            # Если заклинание не выбрано - все кнопки disabled
...
def display_player_game_block(block: Dict):
    ...
    with st.container():
        ...
        with col_repeat:
            if block['attempts'] >= block['max_attempts']:
                # Проверяем, есть ли уже запрос на повтор
                existing_request = next((r for r in shared_data["client_requests"] 
...
def main():
    ...
    st.sidebar.write(f"**Роль:** {'🎮 Игрок' if st.session_state.user_type == 'player' else '👑 Мастер' if st.session_state.user_type == 'host' else '👤 Гость'}")
    ...
    if st.session_state.user_type in ["player", "host"]:
        if st.sidebar.button("🚪 Выйти", use_container_width=True):
            # Удаляем пользователя из общих данных
            if st.session_state.user_id in shared_data["users"]:
